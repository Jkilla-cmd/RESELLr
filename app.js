...
function setRestoreStatus(msg,type="good"){const el=$("#restoreStatus");if(el){el.textContent=msg;el.className="restore-status "+type}else alert(msg)}
function asArray(v){return Array.isArray(v)?v:[]}
function findFirstArray(obj,names){
  for(const name of names){if(Array.isArray(obj?.[name]))return obj[name]}
  return []
}
function normalizeItem(r,mode){
...
function parseCsv(text){
  ...
  const split=line=>{
    ...
    for(let i=0;i<line.length;i++){const ch=line[i];if(ch=='"'&&line[i+1]=='"'){cur+='"';i++}else if(ch=='"'){q=!q}else if(ch==","&&!q){a.push(cur);cur=""}else cur+=ch}
    a.push(cur);return a
  }
  const heads=split(lines[0]).map(h=>h.trim())
  return lines.slice(1).map(line=>{const vals=split(line);const o={};heads.forEach((h,i)=>o[h]=vals[i]??"");return o})
}
function classifyCsvRows(rows){
  let soldRows=[], activeRows=[]
  for(const r of rows){
    const keys=Object.keys(r).join(" ").toLowerCase()
    const hasSold=keys.includes("sold")||r.soldDate||r["Sold Date"]||r.saleDate
    const item=normalizeItem(r,hasSold?"sold":"active")
    if(!item)continue
    if(hasSold)soldRows.push(item);else activeRows.push(item)
  }
  return {activeRows,soldRows}
}
function extractBackupData(j){
  let activeRows=[], soldRows=[], holdRows=[]
  if(Array.isArray(j)){
    activeRows=j.map(x=>normalizeItem(x,"active")).filter(Boolean)
    return {activeRows,soldRows,holdRows}
  }
  activeRows=findFirstArray(j,["activeInventory","inventory","inventoryData","items","active","listings","products"]).map(x=>normalizeItem(x,"active")).filter(Boolean)
  soldRows=findFirstArray(j,["soldInventory","sold","soldData","sales","soldItems","completed"]).map(x=>normalizeItem(x,"sold")).filter(Boolean)
  holdRows=findFirstArray(j,["inventoryHolds","holds","holdData","heldItems"]).map(x=>normalizeItem(x,"hold")).filter(Boolean)

  // Some older backups may wrap data inside another object
  for(const key of Object.keys(j||{})){
    const v=j[key]
    if(v&&typeof v==="object"&&!Array.isArray(v)){
      if(!activeRows.length) activeRows=findFirstArray(v,["activeInventory","inventory","inventoryData","items","active","listings","products"]).map(x=>normalizeItem(x,"active")).filter(Boolean)
      if(!soldRows.length) soldRows=findFirstArray(v,["soldInventory","sold","soldData","sales","soldItems","completed"]).map(x=>normalizeItem(x,"sold")).filter(Boolean)
      if(!holdRows.length) holdRows=findFirstArray(v,["inventoryHolds","holds","holdData","heldItems"]).map(x=>normalizeItem(x,"hold")).filter(Boolean)
    }
  }
  return {activeRows,soldRows,holdRows}
}
  ...
  reader.onload=()=>{
    try{
      ...
      if(file.name.toLowerCase().endsWith(".csv")){
        ...
        activeRows=parsed.activeRows;soldRows=parsed.soldRows
      }else{
        const j=JSON.parse(text)
        const data=extractBackupData(j)
        activeRows=data.activeRows;soldRows=data.soldRows;holdRows=data.holdRows
      }
      setActive(activeRows);setSold(soldRows);setHolds(holdRows)
