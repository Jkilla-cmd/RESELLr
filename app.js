...
/* ===== v223: dashboard sales snapshot filters ===== */
function snapshotYears(){
  const yrs=[...new Set(sold().map(r=>dateOf(r)?.getFullYear()).filter(Boolean))].sort((a,b)=>b-a);
  return yrs.length?yrs:[new Date().getFullYear()];
}

function setupSnapshotFilters(){
  const ySel=$("#snapshotYearFilter");
  const mSel=$("#snapshotMonthFilter");
  if(!ySel || !mSel)return;
  ...
  if([...ySel.options].some(o=>o.value===oldY)) ySel.value=oldY;
  else ySel.value=String(new Date().getFullYear());

  if(!mSel.options.length){
    mSel.innerHTML='<option value="all">All Months</option>'+Array.from({length:12},(_,i)=>`<option value="${i+1}">${monthName(i+1)}</option>`).join("");
    mSel.value="all";
  }
}

function snapshotFilteredRows(){
  const y=$("#snapshotYearFilter")?.value||String(new Date().getFullYear());
  const m=$("#snapshotMonthFilter")?.value||"all";
...
function platformMatch(row,name){
  const p=platform(row).toLowerCase();
  if(name==="mercari")return p.includes("mercari");
  if(name==="ebay")return p.includes("ebay") || p.includes("e-bay");
  if(name==="private")return p.includes("private");
  return false;
}

function renderSalesSnapshot(){
  setupSnapshotFilters();
  const rows=snapshotFilteredRows();
  const income=rows.reduce((x,r)=>x+price(r),0);
  const prof=rows.reduce((x,r)=>x+profit(r),0);
  const cp=rows.reduce((x,r)=>x+cost(r)+profit(r),0);

  if($("#snapshotItemsSold")) $("#snapshotItemsSold").textContent=rows.length;
  if($("#snapshotCostProfit")) $("#snapshotCostProfit").textContent=money(cp);
  if($("#snapshotIncome")) $("#snapshotIncome").textContent=money(income);
  if($("#snapshotProfit")) $("#snapshotProfit").textContent=money(prof);

  const groups={
    mercari:rows.filter(r=>platformMatch(r,"mercari")),
    ebay:rows.filter(r=>platformMatch(r,"ebay")),
    private:rows.filter(r=>platformMatch(r,"private"))
  };
  const sums={
    mercari:groups.mercari.reduce((x,r)=>x+price(r),0),
    ebay:groups.ebay.reduce((x,r)=>x+price(r),0),
    private:groups.private.reduce((x,r)=>x+price(r),0)
  };
  const max=Math.max(1,sums.mercari,sums.ebay,sums.private);

  if($("#snapshotMercari")) $("#snapshotMercari").textContent=money(sums.mercari);
  if($("#snapshotEbay")) $("#snapshotEbay").textContent=money(sums.ebay);
  if($("#snapshotPrivate")) $("#snapshotPrivate").textContent=money(sums.private);
  if($("#snapshotMercariCount")) $("#snapshotMercariCount").textContent=`${groups.mercari.length} sale${groups.mercari.length===1?"":"s"}`;
  if($("#snapshotEbayCount")) $("#snapshotEbayCount").textContent=`${groups.ebay.length} sale${groups.ebay.length===1?"":"s"}`;
  if($("#snapshotPrivateCount")) $("#snapshotPrivateCount").textContent=`${groups.private.length} sale${groups.private.length===1?"":"s"}`;
  if($("#snapshotMercariBar")) $("#snapshotMercariBar").style.width=Math.round(sums.mercari/max*100)+"%";
  if($("#snapshotEbayBar")) $("#snapshotEbayBar").style.width=Math.round(sums.ebay/max*100)+"%";
  if($("#snapshotPrivateBar")) $("#snapshotPrivateBar").style.width=Math.round(sums.private/max*100)+"%";
}

document.addEventListener("change",function(e){
  if(e.target && (e.target.id==="snapshotYearFilter" || e.target.id==="snapshotMonthFilter")){
    renderSalesSnapshot();
  }
},true);

const oldRenderV223=render;
render=function(){
  oldRenderV223();
  renderSalesSnapshot();
};
setTimeout(renderSalesSnapshot,500);


/* ===== v224: dashboard GIF upload/persist ===== */
function renderDashboardGif(){
  const preview=$("#dashboardGifPreview");
  if(!preview)return;
  const data=localStorage.getItem("resellr_dashboard_gif");
  if(data){
    preview.innerHTML=`<img src="${data}" alt="Dashboard GIF">`;
  }else{
    preview.innerHTML=`<span id="dashboardGifEmpty">Upload GIF</span>`;
  }
}

function setupDashboardGif(){
  const input=$("#dashboardGifUpload");
  if(input && !input.dataset.bound){
    ...
    input.addEventListener("change",e=>{
      ...
      const reader=new FileReader();
      reader.onload=()=>{
        localStorage.setItem("resellr_dashboard_gif",String(reader.result||""));
        renderDashboardGif();
      };
...
const oldRenderV224=render;
render=function(){
  oldRenderV224();
  setupDashboardGif();
};
setTimeout(setupDashboardGif,400);


/* ===== v225: snug GIF hover controls + double-click clear ===== */
function renderDashboardGif(){
  const preview=$("#dashboardGifPreview");
  const card=$("#dashboardGifCard");
  if(!preview)return;
  const data=localStorage.getItem("resellr_dashboard_gif");
  if(data){
    preview.innerHTML=`<img src="${data}" alt="Dashboard GIF">`;
    card?.classList.add("has-gif");
  }else{
    preview.innerHTML=`<span id="dashboardGifEmpty">Upload GIF</span>`;
    card?.classList.remove("has-gif");
  }
}

function setupDashboardGif(){
  const input=$("#dashboardGifUpload");
  if(input && !input.dataset.boundV225){
    input.dataset.boundV225="1";
    input.addEventListener("change",e=>{
      const file=e.target.files&&e.target.files[0];
      if(!file)return;
      const reader=new FileReader();
  ...
  const clear=$("#dashboardGifClear");
  if(clear && !clear.dataset.boundV225){
    clear.dataset.boundV225="1";
  ...
  const card=$("#dashboardGifCard");
  if(card && !card.dataset.boundDblClear){
    card.dataset.boundDblClear="1";
    card.addEventListener("dblclick",e=>{
      if(localStorage.getItem("resellr_dashboard_gif")){
        localStorage.removeItem("resellr_dashboard_gif");
        renderDashboardGif();
      }
    });
...
/* ===== v226: force new logo + vintage gif card class ===== */
function forceNewLogoV226(){
  document.querySelectorAll(".brand").forEach(b=>{
    b.innerHTML='<img class="brand-logo" src="assets/branding/logo-full.svg" alt="RESELLr">';
  });
  document.querySelectorAll(".login-logo-wrap").forEach(b=>{
    b.innerHTML='<img src="assets/branding/login-logo.svg" alt="RESELLr" class="login-logo">';
  });
}
function renderDashboardGif(){
  const preview=$("#dashboardGifPreview");
...
const oldRenderV226=render;
render=function(){
  oldRenderV226();
  forceNewLogoV226();
  renderDashboardGif();
};
setTimeout(()=>{forceNewLogoV226();renderDashboardGif();},300);


/* ===== v227: self-healing dashboard snapshot/gif + sold action final fix ===== */
function ensureDashboardCardsV227(){
  const dash=$("#dashboard");
  if(!dash)return;

  if(!$("#snapshotSalesCard")){
    const snap=document.createElement("div");
    snap.className="snapshot-card";
    snap.id="snapshotSalesCard";
    snap.innerHTML=`<div class="card-head"><h3>Sales Snapshot</h3><div class="inline-tools"><select id="snapshotYearFilter"></select><select id="snapshotMonthFilter"></select></div></div>
    <div class="snapshot-grid"><div><span>Items Sold</span><b id="snapshotItemsSold">0</b></div><div><span>Cost + Profit</span><b id="snapshotCostProfit">$0</b></div><div><span>Total Income</span><b id="snapshotIncome">$0</b></div><div><span>Total Profit</span><b id="snapshotProfit">$0</b></div></div>
    <div class="platform-snapshot"><div class="platform-row"><span>Mercari</span><b id="snapshotMercari">$0</b><small id="snapshotMercariCount">0 sales</small></div><div class="platform-bar"><i id="snapshotMercariBar"></i></div><div class="platform-row"><span>eBay</span><b id="snapshotEbay">$0</b><small id="snapshotEbayCount">0 sales</small></div><div class="platform-bar"><i id="snapshotEbayBar"></i></div><div class="platform-row"><span>Private Sale</span><b id="snapshotPrivate">$0</b><small id="snapshotPrivateCount">0 sales</small></div><div class="platform-bar"><i id="snapshotPrivateBar"></i></div></div>`;
    const gif=$("#dashboardGifCard");
    if(gif) dash.insertBefore(snap,gif);
    else dash.insertBefore(snap,dash.querySelector(".monthly-card")||dash.querySelector(".lower-grid"));
  }

  if(!$("#dashboardGifCard")){
    const gif=document.createElement("div");
    gif.className="dashboard-gif-card";
    gif.id="dashboardGifCard";
    gif.innerHTML=`<div class="dashboard-gif-preview" id="dashboardGifPreview"><span id="dashboardGifEmpty">Upload GIF</span></div><div class="dashboard-gif-controls"><div><b>Dashboard GIF</b><small>Vintage dashboard motion</small></div><label class="gif-upload-btn">Upload GIF<input id="dashboardGifUpload" type="file" accept="image/gif,image/*" hidden></label><button type="button" id="dashboardGifClear">Clear</button></div>`;
    dash.insertBefore(gif,dash.querySelector(".monthly-card")||dash.querySelector(".lower-grid"));
  }
}

function renderSalesSnapshot(){
  ensureDashboardCardsV227();
  const ySel=$("#snapshotYearFilter"), mSel=$("#snapshotMonthFilter");
  if(!ySel||!mSel)return;
  const yrs=[...new Set(sold().map(r=>dateOf(r)?.getFullYear()).filter(Boolean))].sort((a,b)=>b-a);
  const oldY=ySel.value;
  ySel.innerHTML='<option value="all">All Years</option>'+(yrs.length?yrs:[new Date().getFullYear()]).map(y=>`<option value="${y}">${y}</option>`).join("");
  if([...ySel.options].some(o=>o.value===oldY)) ySel.value=oldY; else ySel.value=String(new Date().getFullYear());
  if(!mSel.options.length)mSel.innerHTML='<option value="all">All Months</option>'+Array.from({length:12},(_,i)=>`<option value="${i+1}">${monthName(i+1)}</option>`).join("");
  const y=ySel.value,m=mSel.value||"all";
  const rows=sold().filter(r=>{const d=dateOf(r);return d&&(y==="all"||d.getFullYear()===Number(y))&&(m==="all"||d.getMonth()+1===Number(m))});
  const income=rows.reduce((x,r)=>x+price(r),0), prof=rows.reduce((x,r)=>x+profit(r),0), cp=rows.reduce((x,r)=>x+cost(r)+profit(r),0);
  $("#snapshotItemsSold").textContent=rows.length; $("#snapshotCostProfit").textContent=money(cp); $("#snapshotIncome").textContent=money(income); $("#snapshotProfit").textContent=money(prof);
  const groups={mercari:rows.filter(r=>platform(r).toLowerCase().includes("mercari")),ebay:rows.filter(r=>platform(r).toLowerCase().includes("ebay")),private:rows.filter(r=>platform(r).toLowerCase().includes("private"))};
  const sums={mercari:groups.mercari.reduce((x,r)=>x+price(r),0),ebay:groups.ebay.reduce((x,r)=>x+price(r),0),private:groups.private.reduce((x,r)=>x+price(r),0)};
  const max=Math.max(1,sums.mercari,sums.ebay,sums.private);
  $("#snapshotMercari").textContent=money(sums.mercari); $("#snapshotEbay").textContent=money(sums.ebay); $("#snapshotPrivate").textContent=money(sums.private);
  $("#snapshotMercariCount").textContent=`${groups.mercari.length} sales`; $("#snapshotEbayCount").textContent=`${groups.ebay.length} sales`; $("#snapshotPrivateCount").textContent=`${groups.private.length} sales`;
  $("#snapshotMercariBar").style.width=Math.round(sums.mercari/max*100)+"%"; $("#snapshotEbayBar").style.width=Math.round(sums.ebay/max*100)+"%"; $("#snapshotPrivateBar").style.width=Math.round(sums.private/max*100)+"%";
}

function renderDashboardGif(){
  ensureDashboardCardsV227();
  const preview=$("#dashboardGifPreview"), card=$("#dashboardGifCard");
  if(!preview)return;
  const data=localStorage.getItem("resellr_dashboard_gif");
  if(data){preview.innerHTML=`<img src="${data}" alt="Dashboard GIF">`;card?.classList.add("has-gif")}
  else{preview.innerHTML='<span id="dashboardGifEmpty">Upload GIF</span>';card?.classList.remove("has-gif")}
}

function setupDashboardGif(){
  ensureDashboardCardsV227();
  const input=$("#dashboardGifUpload");
  if(input&&!input.dataset.v227){input.dataset.v227="1";input.addEventListener("change",e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>{localStorage.setItem("resellr_dashboard_gif",String(r.result||""));renderDashboardGif()};r.readAsDataURL(f)})}
  const card=$("#dashboardGifCard");
  if(card&&!card.dataset.v227){card.dataset.v227="1";card.addEventListener("dblclick",()=>{if(localStorage.getItem("resellr_dashboard_gif")){localStorage.removeItem("resellr_dashboard_gif");renderDashboardGif()}})}
}

function deleteSoldActionV227(index){
  const rows=sold(), item=rows[index];
  if(!item)return;
  if(confirm(`Delete sold record for "${title(item)}"?`)){
    rows.splice(index,1);setSold(rows);render();
  }
}
function moveSoldToInventoryActionV227(index){
  const rows=sold(), item=rows[index];
  if(!item)return;
  const a=active();
  a.unshift({...item,status:"active",addedAt:new Date().toISOString()});
  rows.splice(index,1);setSold(rows);setActive(a);render();showPage("inventory");
}
...
renderSoldRows=function(){
  ...
  const rows=getSoldFilteredRows().map(r=>({r,i:base.indexOf(r)}));
  updateSoldKpisFromRows(rows.map(x=>x.r));
  $("#soldRows").innerHTML=rows.map(({r,i})=>{const p=price(r),pr=profit(r),m=p?Math.round(pr/p*100):0;return `<tr><td><div class="item-cell"><div class="thumb"></div><div>${esc(title(r))}<small>${esc(platform(r))}</small></div></div></td><td>${fmt(dateOf(r))}</td><td>${money(p)}</td><td>${money(cost(r))}</td><td class="${pr>=0?"profit":"loss"}">${money(pr)}</td><td class="margin">${m}%</td><td><div class="row-actions"><button type="button" data-sold-final="move" data-index="${i}" class="icon-action" title="Move back to inventory">▣</button><button type="button" data-sold-final="delete" data-index="${i}" class="icon-action delete-btn" title="Delete sold item">×</button></div></td></tr>`}).join("");
};

document.addEventListener("click",function(e){
  const btn=e.target.closest&&e.target.closest("[data-sold-final]");
  if(!btn)return;
  e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
  const i=Number(btn.dataset.index);
  if(btn.dataset.soldFinal==="delete")deleteSoldActionV227(i);
  if(btn.dataset.soldFinal==="move")moveSoldToInventoryActionV227(i);
},true);

document.addEventListener("change",function(e){
  if(e.target&&["snapshotYearFilter","snapshotMonthFilter"].includes(e.target.id))renderSalesSnapshot();
},true);

const oldRenderV227=render;
render=function(){
  ensureDashboardCardsV227();
  oldRenderV227();
  renderSalesSnapshot();
